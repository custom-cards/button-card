...
`;let Gt = class extends at {
  static get styles() {
    ...
    if (!this.config.show_entity_picture || !t && !e && !this.config.entity_picture) return;let i, n;return (n = e && e.entity_picture_template ? e.entity_picture_template : this.config.entity_picture_template) ? this._evalTemplate(t, n) : (e && e.entity_picture ? i = e.entity_picture : this.config.entity_picture ? i = this.config.entity_picture : t && (i = t.attributes && t.attributes.entity_picture ? t.attributes.entity_picture : void 0), i);
  }_buildStyleGeneric(t, e) {
    let i = {};if (this.config.styles && this.config.styles[e] && (i = Object.assign(i, ...this.config.styles[e])), t && t.styles && t.styles[e]) {
      let n = {};n = Object.assign(n, ...t.styles[e]), i = Object.assign({}, i, n);
    }return i;
  }_buildName(t, e) {
    if (!1 === this.config.show_name) return;let i, n;return (n = e && e.name_template ? e.name_template : this.config.name_template) ? this._evalTemplate(t, n) : (e && e.name ? i = e.name : this.config.name ? i = this.config.name : t && (i = t.attributes && t.attributes.friendly_name ? t.attributes.friendly_name : (s = t.entity_id).substr(s.indexOf(".") + 1)), i);var s;
  }_buildStateString(t) {
    ...
    if (!this.config.show_label) return;let i, n;return (n = e && e.label_template ? e.label_template : this.config.label_template) ? this._evalTemplate(t, n) : i = e && e.label ? e.label : this.config.label;
  }_isClickable(t) {
    ...
    if (!t) throw new Error("Invalid configuration");const e = function () {
      let t = document.querySelector("home-assistant");if (t = (t = (t = (t = (t = (t = (t = (t = t && t.shadowRoot) && t.querySelector("home-assistant-main")) && t.shadowRoot) && t.querySelector("app-drawer-layout partial-panel-resolver")) && t.shadowRoot || t) && t.querySelector("ha-panel-lovelace")) && t.shadowRoot) && t.querySelector("hui-root")) {
        ...
        n = i.template;for (; n && e.config.button_card_templates && e.config.button_card_templates[n];) i = Lt(e.config.button_card_templates[n], i), n = e.config.button_card_templates[n].template;this.config = Object.assign({ tap_action: { action: "toggle" }, hold_action: { action: "none" }, dbltap_action: { action: "none" }, layout: "vertical", size: "40%", color_type: "icon", show_name: !0, show_state: !1, show_icon: !0, show_units: !0, show_label: !1, show_entity_picture: !1 }, i), this.config.default_color = "var(--primary-text-color)", "icon" !== this.config.color_type ? this.config.color_off = "var(--paper-card-background-color)" : this.config.color_off = "var(--paper-item-icon-color)", this.config.color_on = "var(--paper-item-icon-active-color)";
  }getCardSize() {
    return 3;
  }_handleTap(t) {
